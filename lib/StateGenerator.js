 ...
 *
 * NOTE: Pattern inversion is powerful, but dangerous.  It will turn the inverted
 * node into the catch-all for that state in the FSM, this means that there can
 * be only one per level, and there should be no catch-all, because you shouldn't
 * specify an inverted step unless you have a plan for the exception.  The most
 * common use-case for an inversion is as a repeated step until some end criteria
 * is met.
 */
StateGenerator.prototype._addChildrenToNode = function (steps, currentNode, prefix, lexeme) {
  ...
  if (!nextNode) {
    ...
    nextNode.setAttribute('inverted', step.inverted);

    nextNode.setAttribute('criteria', function (input) {
      // Use the closure to keep the values
      return (step.inverted ? input.token !== token : input.token === token);
    });

    // Add the default transition as a child to this node
    if (!step.inverted && !(peekAheadStep && peekAheadStep.inverted)) {
  ...
  this._addChildrenToNode(steps.slice(), nextNode, nextPrefix, lexeme);
  ...
  // step removed)
  if (step.optional) {
    this._addChildrenToNode(steps.slice(), currentNode, nextPrefix, lexeme);
  }
};
