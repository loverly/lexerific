...
Lexeme.prototype._cleanConfig = function validateConfig(config) {
  ...
  if (config.mode === 'string') {
    if (typeof config.pattern !== 'string') {
      this._throwError('Lexeme string mode patterns must be a string');
    }
  ...
  if (config.mode === 'token') {
    ...
    // Loop through the token pattern array and validate
...
/**
 * Provide a public API for looping through the the pattern so we can decouple
 * the pattern from the Array interface.
 */
Lexeme.prototype.forEachStepInPattern = function (handler) {
  if (this._isDefault) {
    this._throwError('Cannot loop through the pattern for the default lexeme, it has not pattern.');
  }

  this._pattern.forEach(handler);
};
